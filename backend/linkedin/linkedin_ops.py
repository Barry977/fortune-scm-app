...
class LinkedInOps:
    ...
    async def _do_login(self, page, email: str, password: str) -> Dict[str, Any]:
        """执行登录流程"""
        self._notify("login", "logging_in", "正在登录...")
        ...
        try:
            ...
            if "checkpoint" in current_url or "challenge" in current_url:
                ...
                for _ in range(60):
                    ...
                    if "feed" in page.url:
                        await self.browser._save_cookies()
                        self._notify("login", "logged_in", "验证通过，登录成功")
                        return {"success": True, "status": "logged_in", "message": "验证通过，登录成功"}
            ...
            return {"success": False, "status": "unknown", "message": f"登录状态未知: {current_url[:100]}"}

        except Exception as e:
            logger.error("[Login] 登录异常: %s", e)
            self._notify("login", "error", str(e)[:200])
            return {"success": False, "status": "error", "message": f"登录异常: {str(e)[:200]}"}
